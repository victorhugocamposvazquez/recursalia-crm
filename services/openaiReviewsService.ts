...
function buildReviewsPrompt(courseTitle: string, count: number): string {
  return `Genera ${count} reseñas de estudiantes para el curso "${courseTitle}".
...
export async function generateReviews(
  ...
  for (let offset = 0; offset < totalCount; offset += BATCH_SIZE) {
    ...
    const prompt = buildReviewsPrompt(courseTitle, count);
